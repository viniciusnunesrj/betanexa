      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {activeProgress.templatesStatus.map(({ template, isOwned, ownedCard }) => {
            ...
            // Synthesis state
            const isSynthesizing = ownedCard?.cardStatus === 'SYNTHESIZING';
            const accumulatedNEX = ownedCard
              ? EconomyService.calculateAccumulatedNEX(ownedCard)
              : 0;
                ...
                <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="p-3 rounded-2xl bg-black/40 border border-white/5 space-y-1.5 font-mono text-xs">
                    <div className="flex items-center justify-between text-slate-400">
                      ...
                      <span className="font-bold text-cyan-400">+{template.synthesisRate} NEX/h</span>
                    </div>
                    <div className="flex items-center justify-between text-slate-400">
                      <span>Capacidade Máxima:</span>
                      <span className="font-bold text-slate-200">{template.synthesisCap} NEX</span>
                    </div>
                  </div>
                      ...
                      <>
                        {isSynthesizing ? (
                          <div className="space-y-2">
                            <button
                              onClick={() => claimCardSynthesis(ownedCard.id)}
                              disabled={accumulatedNEX <= 0}
                            ...
                            >
                              ...
                              <span>Coletar {accumulatedNEX} NEX</span>
                            </button>

                            <button
                              onClick={() => stopCardSynthesis(ownedCard.id)}
                              className="w-full py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-400 hover:text-white text-[11px] font-mono transition-colors"
                            >
                              Interromper Síntese
                            </button>
                          </div>
                          ...
                          <div className="space-y-2">
                            <button
                              onClick={() => synthesizeCard(ownedCard.id)}
                            ...
                            >
                              ...
                              <span>Iniciar Síntese NEX</span>
                            </button>

                            {ownedCard.cardStatus === 'FREE' && (
                              <button
                                onClick={() => setSellingCard(ownedCard)}
